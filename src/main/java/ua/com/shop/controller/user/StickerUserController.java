...
public class StickerUserController {
	...
	public String show(Model model, @PageableDefault Pageable pageable,
			...
			@ModelAttribute("filter") StickerFilter filter) {
		model.addAttribute("page", stickerService.findAll(pageable, filter));
	...
	public String buy(Model model, @CookieValue(value = "userId") Long userId,
		...
		return "redirect:/sticker" + getParams(pageable, filter);
	}
	...
	public String details(Model model,
			...
			@PathVariable Long chalkId, @PageableDefault Pageable pageable,
			@ModelAttribute("filter") StickerFilter filter) {
		model.addAttribute("sticker", stickerService.findOne(chalkId));
		...
		model.addAttribute("gloves", cartItemService.findGloveInCart(cartId));
		model.addAttribute("userForm", new User());
		model.addAttribute("reviews", reviewsService.findByStickers(chalkId));
		return "user-stickerCard";
	}
	...
	public String toBascet(Model model,
			...
			@ModelAttribute("filter") StickerFilter filter) {
		userService.addToCartItemSticker(userId, chalkId);
		return "redirect:/sticker/details/{chalkId}";
	}

	private String getParams(Pageable pageable, StickerFilter filter) {
		...
		if (!filter.getNameSticker().isEmpty()) {
			buffer.append("&name=");
			buffer.append(filter.getNameSticker());
		}
