...
@Controller
@RequestMapping("/chalk")
public class ChalkUserController {
	...
	public ChalkFilter getFilter() {
		return new ChalkFilter();
	}

	@GetMapping
	public String show(Model model, @PageableDefault Pageable pageable,
			@CookieValue(defaultValue = "0", name = "userId") Long id,
			@ModelAttribute("filter") ChalkFilter filter) {
		model.addAttribute("page", chalkService.findAll(pageable, filter));
	...
	public String buy(Model model, @CookieValue(value = "userId") Long userId,
		...
		return "redirect:/chalk" + getParams(pageable, filter);
	}
	...
	public String details(Model model,
			...
			@PathVariable Long chalkId, @PageableDefault Pageable pageable,
			@ModelAttribute("filter") ChalkFilter filter) {
		model.addAttribute("chalk", chalkService.findOne(chalkId));
		...
		model.addAttribute("gloves", cartItemService.findGloveInCart(cartId));
		model.addAttribute("userForm", new User());
		model.addAttribute("reviews", reviewsService.findByChalk(chalkId));
		return "user-chalkCard";
	}
	...
	public String toBascet(Model model,
			...
			@ModelAttribute("filter") ChalkFilter filter) {
		userService.addToCartItemChalk(userId, chalkId);
		return "redirect:/chalk/details/{chalkId}";
	}

	private String getParams(Pageable pageable, ChalkFilter filter) {
		...
		if (!filter.getNameChalk().isEmpty()) {
			buffer.append("&name=");
			buffer.append(filter.getNameChalk());
		}
