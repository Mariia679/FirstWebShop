...
import ua.com.shop.validator.ShaftValidator;

@Controller
@RequestMapping("/admin/shaft")
@SessionAttributes("shaft")
public class ShaftController {
	...
	protected void initBinder(WebDataBinder binder) {
		binder.setValidator(new ShaftValidator(shaftService));
	}

	@ModelAttribute("shaft")
	public Shaft getForm() {
		return new Shaft();
	}
	...
	public String show(Model model, @PageableDefault Pageable pageable,
		...
		model.addAttribute("page", shaftService.findAll(filter, pageable));
	...
	public String update(@PathVariable Long id, Model model,
			...
			@ModelAttribute("filter") SimpleFilter filter,@CookieValue(defaultValue = "0", name = "userId") Long userId) {
		model.addAttribute("shaft", shaftService.findOne(id));
	...
	public String delete(@PathVariable Long id,
		...
		commodityService.setNullShaft(id);
		shaftService.delete(id);
		return "redirect:/admin/shaft" + getParams(pageable, filter);
	}
	...
	@PostMapping
	public String save(@ModelAttribute("shaft") @Valid Shaft shaft,
