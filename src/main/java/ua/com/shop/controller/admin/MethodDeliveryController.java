...
import ua.com.shop.validator.MethodDeliveryValidator;

@Controller
@RequestMapping("/admin/methodDelivery")
@SessionAttributes("methodDelivery")
public class MethodDeliveryController {

	@Autowired
	private MethodDeliveryService methodDeliveryService;
	
	@Autowired
	private CartItemService cartItemService;

	@InitBinder("methodDelivery")
	protected void initBinder(WebDataBinder binder) {
		binder.setValidator(new MethodDeliveryValidator(methodDeliveryService));
	}

	@ModelAttribute("methodDelivery")
	public MethodDelivery getForm() {
		return new MethodDelivery();
	}
	...
	public String show(Model model, @PageableDefault Pageable pageable,
		...
		model.addAttribute("page",
				methodDeliveryService.findAll(filter, pageable));
	...
	public String update(@PathVariable Long id, Model model,
			...
			@ModelAttribute("filter") SimpleFilter filter,@CookieValue(defaultValue = "0", name = "userId") Long userId) {
		model.addAttribute("methodDelivery", methodDeliveryService.findOne(id));
	...
	public String delete(@PathVariable Long id,
		...
		methodDeliveryService.delete(id);
		return "redirect:/admin/methodDelivery" + getParams(pageable, filter);
	}
	...
	@PostMapping
	public String save(
			@ModelAttribute("methodDelivery") @Valid MethodDelivery methodDelivery,
		...
		if (br.hasErrors())
			return show(model, pageable, filter,userId);
		methodDeliveryService.save(methodDelivery);
		status.setComplete();
		return "redirect:/admin/methodDelivery" + getParams(pageable, filter);
	}
