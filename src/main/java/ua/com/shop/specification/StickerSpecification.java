...
public class StickerSpecification implements Specification<Sticker> {

	private final StickerFilter filter;

	private final List<Predicate> predicates = new ArrayList<>();

	private static final Pattern REG = Pattern.compile("^[0-9]+$");

	public StickerSpecification(StickerFilter filter) {
		this.filter = filter;
	...
	@Override
	public Predicate toPredicate(Root<Sticker> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByPrice(Root<Sticker> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByName(Root<Sticker> root, CriteriaQuery<?> query) {
		if (!filter.getNameSticker().isEmpty()) {
			predicates.add(root.get("name").in(filter.getNameSticker()));
		}
	}
}
