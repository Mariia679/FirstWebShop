...
public class ChalkSpecification implements Specification<Chalk> {

	private final ChalkFilter filter;

	private final List<Predicate> predicates = new ArrayList<>();

	private static final Pattern REG = Pattern.compile("^[0-9]+$");

	public ChalkSpecification(ChalkFilter filter) {
		this.filter = filter;
	...
	@Override
	public Predicate toPredicate(Root<Chalk> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByPrice(Root<Chalk> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByName(Root<Chalk> root, CriteriaQuery<?> query) {
		if (!filter.getNameChalk().isEmpty()) {
			predicates.add(root.get("name").in(filter.getNameChalk()));
		}
	}
}
