...
public class GloveSpecification implements Specification<Glove> {

	private final GloveFilter filter;

	private final List<Predicate> predicates = new ArrayList<>();

	private static final Pattern REG = Pattern.compile("^[0-9]+$");

	public GloveSpecification(GloveFilter filter) {
		this.filter = filter;
	...
	@Override
	public Predicate toPredicate(Root<Glove> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByPrice(Root<Glove> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByName(Root<Glove> root, CriteriaQuery<?> query) {
		if (!filter.getNameGlove().isEmpty()) {
			predicates.add(root.get("name").in(filter.getNameGlove()));
		}
	}
}
